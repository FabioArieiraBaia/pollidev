...
async function ensureReactBuilt() {
    // Always rebuild React components to ensure latest changes are included
    // This is necessary because React components are compiled separately from TypeScript
    // Only skip if explicitly disabled via environment variable
    if (process.env.VSCODE_SKIP_BUILD_REACT !== '1') {
