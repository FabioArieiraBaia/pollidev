...
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	private async _runChatAgent({
		...
		// Se o modo for multi-agent, verificar se está habilitado
		if (chatMode === 'multi-agent') {
			const multiAgentSettings = this._settingsService.state.globalSettings.multiAgentSettings
			
			// Se multi-agent não está habilitado ou não está implementado, usar fluxo normal do agente
			if (!multiAgentSettings.enabled) {
				// Fallback para modo 'agent' normal
				// Continue com o fluxo normal abaixo
			} else {
				// Multi-agent está habilitado, mas a implementação está incompleta
				// Por enquanto, usar o fluxo normal do agente como fallback
				// TODO: Implementar multi-agent completo quando estiver pronto
				this.logService.warn('[ChatThreadService] Multi-agent enabled but not fully implemented, using normal agent flow')
				// Continue com o fluxo normal abaixo
			}
		}
