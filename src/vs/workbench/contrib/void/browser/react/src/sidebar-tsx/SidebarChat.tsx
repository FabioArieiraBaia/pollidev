...
import { getModelCapabilities, getIsReasoningEnabledState, modelSupportsCapability } from '../../../../common/modelCapabilities.js';
import { AlertTriangle, File, Ban, Check, ChevronRight, Dot, FileIcon, Pencil, Undo, Undo2, X, Flag, Copy as CopyIcon, Info, CirclePlus, Ellipsis, CircleEllipsis, Folder, ALargeSmall, TypeOutline, Text, Globe, Image, Mic, Video } from 'lucide-react';
...
import { AgentChecklist, ActiveAgentsView } from '../multi-agent-tsx/index.js';
...
const ChatModeDropdown = ({ className }: { className: string }) => {
	...
	const options: ChatMode[] = useMemo(() => ['normal', 'gather', 'agent', 'multi-agent'], [])
...
const nameOfAgentSuperpowerMode: Record<import('../../../../common/voidSettingsTypes.js').AgentSuperpowerMode, string> = {
	'plan': 'Plan',
	'debug': 'Debug',
	'ask': 'Ask',
}

const detailOfAgentSuperpowerMode: Record<import('../../../../common/voidSettingsTypes.js').AgentSuperpowerMode, string> = {
	'plan': 'Creates detailed plans before executing',
	'debug': 'Focuses on identifying and fixing errors',
	'ask': 'Answers questions without executing actions',
}

const AgentSuperpowerDropdown = ({ className }: { className: string }) => {
	const accessor = useAccessor()
	const voidSettingsService = accessor.get('IVoidSettingsService')
	const settingsState = useSettingsState()
	const chatMode = settingsState.globalSettings.chatMode

	// Only show when in agent or multi-agent mode
	if (chatMode !== 'agent' && chatMode !== 'multi-agent') return null

	const options: import('../../../../common/voidSettingsTypes.js').AgentSuperpowerMode[] = useMemo(() => ['plan', 'debug', 'ask'], [])
	const currentMode = settingsState.globalSettings.agentSuperpowerMode || 'plan'

	const onChangeOption = useCallback((newVal: import('../../../../common/voidSettingsTypes.js').AgentSuperpowerMode) => {
		voidSettingsService.setGlobalSetting('agentSuperpowerMode', newVal)
	}, [voidSettingsService])
	...
	return <VoidCustomDropdownBox
		...
		onChangeOption={onChangeOption}
		getOptionDisplayName={(val) => `∞ ${nameOfAgentSuperpowerMode[val]}`}
		getOptionDropdownName={(val) => `∞ ${nameOfAgentSuperpowerMode[val]}`}
		getOptionDropdownDetail={(val) => detailOfAgentSuperpowerMode[val]}
		getOptionsEqual={(a, b) => a === b}
	/>
}

const SharedBrowserToggle = ({ className }: { className: string }) => {
	const accessor = useAccessor()
	const sharedBrowserService = accessor.get('ISharedBrowserService') as ISharedBrowserService
	const [isActive, setIsActive] = useState(sharedBrowserService.state.isActive)

	useEffect(() => {
		setIsActive(sharedBrowserService.state.isActive)
	...
	const handleToggle = useCallback(async () => {
		if (isActive) {
			...
			await sharedBrowserService.open()
		}
	}, [isActive, sharedBrowserService])

	return (
		<button
			onClick={handleToggle}
			className={`${className} flex items-center gap-1.5 px-1.5 py-0.5 rounded transition-colors ${
				isActive 
					? 'bg-void-bg-2 border-void-border-1 text-void-fg-1' 
					: 'bg-void-bg-1 border-void-border-2 text-void-fg-3 hover:text-void-fg-2'
			}`}
			title={isActive ? 'Close Shared Browser' : 'Open Shared Browser'}
		>
				...
				{showModelDropdown && (
					<div className='flex flex-col gap-y-1'>
						...
						<div className='flex items-center flex-wrap gap-x-2 gap-y-1 text-nowrap '>
							{featureName === 'Chat' && <AgentSuperpowerDropdown className='text-xs text-void-fg-3 bg-void-bg-1 border border-void-border-2 rounded py-0.5 px-1' />}
							{featureName === 'Chat' && <ChatModeDropdown className='text-xs text-void-fg-3 bg-void-bg-1 border border-void-border-2 rounded py-0.5 px-1' />}
							{featureName === 'Chat' && <SharedBrowserToggle className='text-xs border border-void-border-2 rounded' />}
...
const AttachmentButtons = ({ featureName, attachments, setAttachments }: { featureName: FeatureName, attachments: Attachment[], setAttachments: React.Dispatch<React.SetStateAction<Attachment[]>> }) => {
	...
	const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		Array.from(files).forEach(file => {
			...
			reader.onload = (ev) => {
				setAttachments(prev => [...prev, { type: 'image', file, preview: ev.target?.result as string }])
			}
	...
	const handleVideoSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (!files) return
		Array.from(files).forEach(file => {
			setAttachments(prev => [...prev, { type: 'video', file }])
		})
		e.target.value = ''
	}
		...
		<div className="flex items-center gap-1">
			...
			{supportsVision && (
				<>
					...
					<button
						type="button"
						className="p-1 rounded hover:bg-void-bg-2 text-void-fg-3 hover:text-void-fg-1 transition-colors"
						...
						<div key={idx} className="relative group">
							{att.type === 'image' && att.preview && (
								<img
									src={att.preview}
									alt="attachment"
									className="w-6 h-6 rounded object-cover border border-void-border-2"
								/>
							)}
							{att.type === 'audio' && (
								<div className="w-6 h-6 rounded bg-void-bg-2 flex items-center justify-center border border-void-border-2">
									<Mic size={12} />
								</div>
							)}
							{att.type === 'video' && (
...
const ToolHeaderWrapper = ({
	...
	return (<div className=''>
		<div className={`w-full rounded px-2 py-1 overflow-hidden ${className || 'border border-void-border-3 bg-void-bg-3'}`}>
			{/* header */}
...
const UserMessageComponent = ({ chatMessage, messageIdx, isCheckpointGhost, currCheckpointIdx, _scrollToBottom }: { chatMessage: ChatMessage & { role: 'user' }, messageIdx: number, currCheckpointIdx: number | undefined, isCheckpointGhost: boolean, _scrollToBottom: (() => void) | null }) => {
	...
	>
		<div
            ...
            ${mode === 'edit' ? ''
					: mode === 'display' ? `p-2 flex flex-col text-void-fg-1 overflow-x-auto cursor-pointer void-user-message-clean` : ''
				}
        `}
...
const AssistantMessageComponent = ({ chatMessage, isCheckpointGhost, isCommitted, messageIdx }: { chatMessage: ChatMessage & { role: 'assistant' }, isCheckpointGhost: boolean, messageIdx: number, isCommitted: boolean }) => {
	...
	return <>
		...
		{chatMessage.displayContent &&
			<div className={`${isCheckpointGhost ? 'opacity-50' : ''} void-agent-message-gradient p-3 rounded-lg`}>
				<ProseWrapper>
					<ChatMarkdownRender
...
const CommandTool = ({ toolMessage, type, threadId }: { threadId: string } & ({
	...
	const componentParams: ToolHeaderParams = { title, desc1, desc1Info, isError, icon, isRejected, className: 'border-void-warning/30 bg-void-warning/10' }
...
// PollyHeader Component
interface PollyHeaderProps {
	isRunning?: IsRunningType;
	streamState?: ReturnType<typeof useChatThreadsStreamState>;
}

const PollyHeader = ({ isRunning, streamState }: PollyHeaderProps) => {
	const [glowClass, setGlowClass] = useState('void-logo-glow')
	const [isPulsing, setIsPulsing] = useState(false)
	const [thinkingText, setThinkingText] = useState<string | null>(null)

	useEffect(() => {
		if (isRunning === 'LLM') {
			setGlowClass('void-logo-glow-blue')
			setIsPulsing(true)
			...
			if (toolName) {
				...
				setThinkingText(`Executing ${cleanToolName}...`)
			} else {
				setThinkingText('Executing tool...')
			}
		} else if (isRunning === undefined || isRunning === 'idle') {
			setGlowClass('void-logo-glow')
			setIsPulsing(false)
...
export const SidebarChat = () => {
	...
	const landingPageContent = <div
		ref={sidebarRef}
		className='w-full h-full max-h-full flex flex-col overflow-auto px-4 void-glass-panel-advanced'
	>
		<PollyHeader isRunning={isRunning} streamState={currThreadStreamState} />
		<ErrorBoundary>
			{landingPageInput}
		</ErrorBoundary>
		...
		{/* SharedBrowserView moved to main editor panel via WebviewPanel */}
	</div>
	...
	const threadPageContent = <div
		ref={sidebarRef}
		className='w-full h-full flex flex-col overflow-hidden void-glass-panel-advanced'
	>
		<PollyHeader isRunning={isRunning} streamState={currThreadStreamState} />
		<ErrorBoundary>
			{messagesHTML}
		...
		{/* Agent Plan Summary Card - show when plan exists */}
		{chatMode === 'multi-agent' && currentPlan && (
			<ErrorBoundary>
				<div className="void-agent-card mx-4 my-2">
					<div className="void-flex void-items-center void-gap-2 void-mb-2">
						<span className="void-text-sm void-font-semibold pollidev-gradient">Plan Summary</span>
					</div>
					<div className="void-text-xs void-text-void-fg-3 void-mb-2">
						{currentPlan.tasks.filter(t => t.status === 'pending').length} pending • {currentPlan.tasks.filter(t => t.status === 'completed').length} completed • {currentPlan.tasks.length} total
					</div>
					<div className="void-text-xs void-text-void-fg-2">
						{currentPlan.tasks.slice(0, 3).map((task, idx) => (
							<div key={idx} className="void-flex void-items-center void-gap-2">
								<span className={task.status === 'completed' ? 'void-text-void-fg-4' : 'void-text-void-fg-2'}>
									{task.status === 'completed' ? '✓' : '○'} {task.description}
								</span>
							</div>
						))}
						{currentPlan.tasks.length > 3 && (
							<div className="void-text-void-fg-4">... and {currentPlan.tasks.length - 3} more</div>
						)}
					</div>
				</div>
			</ErrorBoundary>
		...
		{/* Multi-Agent Checklist - show when plan exists */}
		{chatMode === 'multi-agent' && currentPlan && (
			<ErrorBoundary>
				<AgentChecklist
					plan={currentPlan}
					...
					onPlanUpdate={(updatedPlan) => {
						// Update plan in orchestrator
						if (orchestratorService) {
							orchestratorService.refinePlan(threadId, updatedPlan.tasks.map(t => ({
								id: t.id,
								description: t.description,
								userAssignedAgent: t.userAssignedAgent,
								model: t.model,
								selected: t.selected,
							})));
						}
					...
					onExecute={(taskIds) => {
						orchestratorService?.executeSelectedTasks(threadId, taskIds);
					}}
					onExecuteAll={() => {
						if (currentPlan) {
							const allTaskIds = currentPlan.tasks
								.filter(t => t.status === 'pending')
								.map(t => t.id);
							orchestratorService?.executeSelectedTasks(threadId, allTaskIds);
						}
					}}
				/>
			</ErrorBoundary>
		...
		{/* Shared Browser moved to main editor panel via WebviewPanel */}
		<ErrorBoundary>
			{threadPageInput}
		</ErrorBoundary>
	</div>
