...
import { X, RefreshCw, Loader2, Check, Asterisk, Plus, Database } from 'lucide-react'
...
// RAG Indexing Button
const IndexWorkspaceButton = () => {
	const accessor = useAccessor()
	const workspaceContextService = accessor.get('IWorkspaceContextService')
	const ragIndexingService = accessor.get('IRAGIndexingService')
	const ragVectorService = accessor.get('IRAGVectorService')
	const progressService = accessor.get('IProgressService')
	const notificationService = accessor.get('INotificationService')
	const settingsState = useSettingsState()

	const [isIndexing, setIsIndexing] = useState(false)
	const [indexStats, setIndexStats] = useState<{ documentCount: number; chunkCount: number } | null>(null)

	// Carregar estatísticas iniciais
	useEffect(() => {
		const loadStats = () => {
			try {
				const stats = ragIndexingService.getIndexStats()
				setIndexStats(stats)
			} catch (error) {
	...
	const handleIndexWorkspace = async () => {
		...
		if (workspaceFolders.length === 0) {
			notificationService.warn('No workspace folders open')
			return
		}
		...
		try {
			...
			await progressService.withProgress(
				...
				async (progress) => {
					...
					await ragIndexingService.indexWorkspace(folderPaths)

					// Atualizar estatísticas após indexação
					const stats = ragIndexingService.getIndexStats()
					setIndexStats(stats)
				}
			)

			notificationService.info('Workspace indexed successfully for RAG')
		} catch (error) {
			notificationService.error(`Failed to index workspace: ${String(error)}`)
		} finally {
			setIsIndexing(false)
		}
	}

	const isEnabled = settingsState.globalSettings.ragSettings?.enabled ?? false

	// Determinar texto do botão
	let buttonText = 'Manually index all files in workspace for semantic search'
	if (isIndexing) {
		buttonText = 'Indexing workspace...'
	} else if (indexStats && indexStats.documentCount > 0) {
		buttonText = `Workspace indexed: ${indexStats.documentCount} files, ${indexStats.chunkCount} chunks. Click to re-index.`
	}
	...
	return (
		<ButtonLeftTextRightOption
			...
			leftButton={
				<button
					className="flex items-center"
					disabled={isIndexing || !isEnabled}
					onClick={handleIndexWorkspace}
				>
					{isIndexing ? (
						<Loader2 className="size-3 animate-spin" />
					) : indexStats && indexStats.documentCount > 0 ? (
						<RefreshCw className="size-3" />
					) : (
		...
		>
			...
			>
				...
				<div className="text-sm text-void-fg-3 mb-4">
					{type === 'default' ? `${modelName} comes packaged with PolliDev, so you shouldn't need to change these settings.`
						: isUnrecognizedModel
							? `Model not recognized by PolliDev.`
							: `PolliDev recognizes ${modelName} ("${recognizedModelName}").`}
				</div>
...
export const Settings = () => {
	...
	const onDownload = (t: 'Chats' | 'Settings') => {
		...
		if (t === 'Chats') {
			...
			downloadName = 'pollidev-settings.json'
		}
		else {
			dataStr = ''
			downloadName = ''
        ...
        `}
				...
				<main className="flex-1 p-6 select-none">
					...
					<div className='max-w-3xl'>
						...
						<h1 className='text-2xl w-full'>{`PolliDev's Settings`}</h1>
							...
							<div className={shouldShowTab('featureOptions') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<div className='flex flex-col gap-y-8 my-4'>
										...
										<ErrorBoundary>
											<div>
												<h4 className={`text-base`}>Multi-Agent System</h4>
												<div className='text-sm text-void-fg-3 mt-1'>{`Enable multiple AI agents working together to handle complex tasks.`}</div>

												<div className='my-2'>
													{/* Enable Multi-Agent Switch */}
													<div className='flex items-center gap-x-2 my-2'>
														<VoidSwitch
															size='xs'
															value={settingsState.globalSettings.multiAgentSettings?.enabled ?? false}
															onChange={(newVal) => {
																const current = settingsState.globalSettings.multiAgentSettings || { enabled: false, orchestratorModel: 'gemini-large', plannerModel: 'perplexity-reasoning', executorModels: [], enableParallelExecution: true, maxConcurrentAgents: 3, autoApproveTasks: false, maxRetries: 2 }
																voidSettingsService.setGlobalSetting('multiAgentSettings', { ...current, enabled: newVal })
															}}
														/>
														<span className='text-void-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.multiAgentSettings?.enabled ? 'Enabled' : 'Disabled'}</span>
													</div>

													{/* Multi-Agent Configuration */}
													<div className={`my-2 ${!(settingsState.globalSettings.multiAgentSettings?.enabled) ? 'hidden' : ''}`}>
														<div className='text-xs text-void-fg-3 mt-2 mb-1'>Orchestrator Model (coordinates agents):</div>
														<VoidCustomDropdownBox
															options={settingsState._modelOptions
																.filter(o => o.selection.providerName === 'pollinations')
																.filter(o => {
																	// Filter by vision if shared browser is enabled
																	if (settingsState.globalSettings.sharedBrowserEnabled) {
															...
															selectedOption={settingsState.globalSettings.multiAgentSettings?.orchestratorModel || 'gemini-large'}
															onChangeOption={(newVal) => {
																const current = settingsState.globalSettings.multiAgentSettings || { enabled: false, orchestratorModel: 'gemini-large', plannerModel: 'perplexity-reasoning', executorModels: [], enableParallelExecution: true, maxConcurrentAgents: 3, autoApproveTasks: false, maxRetries: 2 }
																voidSettingsService.setGlobalSetting('multiAgentSettings', { ...current, orchestratorModel: newVal })
															}}
															...
															className='text-xs text-void-fg-3 bg-void-bg-1 border border-void-border-1 rounded p-0.5 px-1'
														/>
														<div className='text-xs text-void-fg-3 mt-2 mb-1'>Planner Model (creates plans):</div>
														<VoidCustomDropdownBox
															options={settingsState._modelOptions
																.filter(o => o.selection.providerName === 'pollinations')
																.filter(o => {
																...
																.map(o => o.selection.modelName)}
															selectedOption={settingsState.globalSettings.multiAgentSettings?.plannerModel || 'perplexity-reasoning'}
															onChangeOption={(newVal) => {
																const current = settingsState.globalSettings.multiAgentSettings || { enabled: false, orchestratorModel: 'gemini-large', plannerModel: 'perplexity-reasoning', executorModels: [], enableParallelExecution: true, maxConcurrentAgents: 3, autoApproveTasks: false, maxRetries: 2 }
																voidSettingsService.setGlobalSetting('multiAgentSettings', { ...current, plannerModel: newVal })
															}}
															getOptionDisplayName={(val) => val}
															getOptionDropdownName={(val) => val}
															getOptionDropdownDetail={(val) => 'pollinations'}
															...
															className='text-xs text-void-fg-3 bg-void-bg-1 border border-void-border-1 rounded p-0.5 px-1'
														/>
														<div className='text-xs text-void-fg-3 mt-2 mb-1'>Max Concurrent Agents:</div>
														<VoidSimpleInputBox
															type='number'
															value={(settingsState.globalSettings.multiAgentSettings?.maxConcurrentAgents || 3).toString()}
															onChange={(newVal) => {
																const num = parseInt(newVal) || 1
																const current = settingsState.globalSettings.multiAgentSettings || { enabled: false, orchestratorModel: 'gemini-large', plannerModel: 'perplexity-reasoning', executorModels: [], enableParallelExecution: true, maxConcurrentAgents: 3, autoApproveTasks: false, maxRetries: 2 }
																voidSettingsService.setGlobalSetting('multiAgentSettings', { ...current, maxConcurrentAgents: Math.max(1, Math.min(10, num)) })
															}}
															className='text-xs text-void-fg-3 bg-void-bg-1 border border-void-border-1 rounded p-0.5 px-1 w-20'
														/>
														<div className='flex items-center gap-x-2 my-2 mt-2'>
															<VoidSwitch
																size='xs'
																value={settingsState.globalSettings.multiAgentSettings?.enableParallelExecution ?? true}
																onChange={(newVal) => {
																	const current = settingsState.globalSettings.multiAgentSettings || { enabled: false, orchestratorModel: 'gemini-large', plannerModel: 'perplexity-reasoning', executorModels: [], enableParallelExecution: true, maxConcurrentAgents: 3, autoApproveTasks: false, maxRetries: 2 }
																	voidSettingsService.setGlobalSetting('multiAgentSettings', { ...current, enableParallelExecution: newVal })
																}}
															/>
															<span className='text-void-fg-3 text-xs pointer-events-none'>Parallel Execution</span>
														</div>
													</div>
												</div>
											</div>
										...
										{/* RAG Section - Experimental */}
										<ErrorBoundary>
											<div>
												<h4 className={`text-base`}>
													RAG (Retrieval Augmented Generation) <span className="text-orange-500">[EXPERIMENTAL]</span>
												</h4>
												<div className="text-sm text-void-fg-3 mt-1">
													Enable semantic search to retrieve only relevant context, reducing token usage and improving performance.
												</div>
												...
												<div className="my-2">
													...
													<div className="flex items-center gap-x-2 my-2">
														<VoidSwitch
															size='xs'
															value={settingsState.globalSettings.ragSettings?.enabled ?? false}
															onChange={(newVal) => {
																const current = settingsState.globalSettings.ragSettings || {
																	enabled: false,
																...
																voidSettingsService.setGlobalSetting('ragSettings', { ...current, enabled: newVal });
															}} />
														
														<span className="text-void-fg-3 text-xs pointer-events-none">
															{settingsState.globalSettings.ragSettings?.enabled ? 'Enabled' : 'Disabled'}
														</span>
													</div>

													{/* RAG Configuration */}
													<div className={`my-2 ${!settingsState.globalSettings.ragSettings?.enabled ? "hidden" : ""}`}>
														<div className="text-xs text-void-fg-3 mt-2 mb-1">Similarity Threshold (0.0 - 1.0):</div>
														<VoidSimpleInputBox
															type='number'
															step='0.1'
															min='0'
															max='1'
															value={(settingsState.globalSettings.ragSettings?.similarityThreshold ?? 0.7).toString()}
															onChange={(newVal) => {
																const num = parseFloat(newVal) || 0.7;
																const current = settingsState.globalSettings.ragSettings || {
																	enabled: false,
																	embeddingModel: 'openai-fast',
																	chunkSize: 1000,
																...
																voidSettingsService.setGlobalSetting('ragSettings', { ...current, similarityThreshold: Math.max(0, Math.min(1, num)) });
															}}
															className="text-xs text-void-fg-3 bg-void-bg-1 border border-void-border-1 rounded p-0.5 px-1 w-20" />
														
														<div className="text-xs text-void-fg-3 mt-2 mb-1">Max Results:</div>
														<VoidSimpleInputBox
															type='number'
															min='1'
															max='20'
															value={(settingsState.globalSettings.ragSettings?.maxResults ?? 5).toString()}
															onChange={(newVal) => {
																const num = parseInt(newVal) || 5;
																const current = settingsState.globalSettings.ragSettings || {
																	enabled: false,
																	embeddingModel: 'openai-fast',
																	chunkSize: 1000,
																...
																voidSettingsService.setGlobalSetting('ragSettings', { ...current, maxResults: Math.max(1, Math.min(20, num)) });
															}}
															className="text-xs text-void-fg-3 bg-void-bg-1 border border-void-border-1 rounded p-0.5 px-1 w-20" />
														
														<div className="text-xs text-void-fg-3 mt-2 mb-1">Chunk Size (characters):</div>
														<VoidSimpleInputBox
															type='number'
															min='100'
															max='5000'
															step='100'
															value={(settingsState.globalSettings.ragSettings?.chunkSize ?? 1000).toString()}
															onChange={(newVal) => {
																const num = parseInt(newVal) || 1000;
																const current = settingsState.globalSettings.ragSettings || {
																	enabled: false,
																	embeddingModel: 'openai-fast',
																	chunkSize: 1000,
																...
																voidSettingsService.setGlobalSetting('ragSettings', { ...current, chunkSize: Math.max(100, Math.min(5000, num)) });
															}}
															className="text-xs text-void-fg-3 bg-void-bg-1 border border-void-border-1 rounded p-0.5 px-1 w-20" />
														...
														<div className="flex items-center gap-x-2 my-2 mt-2">
															<VoidSwitch
																size='xs'
																value={settingsState.globalSettings.ragSettings?.autoIndex ?? true}
																onChange={(newVal) => {
																	const current = settingsState.globalSettings.ragSettings || {
																		enabled: false,
																		embeddingModel: 'openai-fast',
																	...
																	voidSettingsService.setGlobalSetting('ragSettings', { ...current, autoIndex: newVal });
																}} />
															
															<span className="text-void-fg-3 text-xs pointer-events-none">Auto Index Files</span>
														</div>

														{/* Manual Index Button */}
														<div className="mt-4 pt-4 border-t border-void-border-1">
															<IndexWorkspaceButton />
														</div>
													</div>
												</div>
											</div>
										...
										<div className='w-full'>
											...
											<div className='text-sm text-void-fg-3 mt-1'>{`Settings that control the visibility of PolliDev suggestions in the code editor.`}</div>
							...
							<div className={`${shouldShowTab('general') ? `` : 'hidden'} flex flex-col gap-12`}>
								...
								<div>
									<ErrorBoundary>
										...
										<h4 className='text-void-fg-3 mb-4'>{`Transfer your editor settings into PolliDev.`}</h4>
								...
								<div>
									...
									<h4 className='text-void-fg-3 mb-4'>{`Transfer PolliDev's settings and chats in and out of PolliDev.`}</h4>
