...
import { IFileService, IFileStat, FileChangesEvent } from '../../../../platform/files/common/files.js';
...
const WORKSPACE_SIZE_SMALL = 100;
const WORKSPACE_SIZE_MEDIUM = 500;
const WORKSPACE_SIZE_LARGE = 2000;

// Restrictive limits for different workspace sizes
const RESTRICTIVE_MAX_DEPTH_SMALL = 3;
const RESTRICTIVE_MAX_ITEMS_PER_DIR_SMALL = 3;

const RESTRICTIVE_MAX_DEPTH_MEDIUM = 2;
const RESTRICTIVE_MAX_ITEMS_PER_DIR_MEDIUM = 2;

const RESTRICTIVE_MAX_DEPTH_LARGE = 2;
const RESTRICTIVE_MAX_ITEMS_PER_DIR_LARGE = 2;

const RESTRICTIVE_MAX_DEPTH_VERY_LARGE = 1;
const RESTRICTIVE_MAX_ITEMS_PER_DIR_VERY_LARGE = 1;
...
/**
 * Estimates workspace size by counting files up to a reasonable limit
 * This is done quickly to avoid performance issues
 */
const estimateWorkspaceSize = async (
	rootURI: URI,
	fileService: IFileService,
	maxFilesToCheck: number = 1000
): Promise<{ fileCount: number; exceededLimit: boolean }> => {
	let fileCount = 0;
	...
	try {
		...
		// Quick estimation: count files in first 2 levels
		const countFiles = async (stat: IFileStat, currentDepth: number, maxDepth: number): Promise<void> => {
			if (fileCount >= maxFilesToCheck) {
				exceededLimit = true;
				return;
			}

			if (stat.isDirectory && stat.children && currentDepth < maxDepth) {
				const eChildren = await resolveChildren(stat.children, fileService);
				...
				for (const child of eChildren) {
					...
					if (!child.isDirectory) {
						...
						if (!shouldExcludeDirectory(child.name)) {
							await countFiles(child, currentDepth + 1, maxDepth);
						}
					}
				}
			}
		...
		// Count files up to 2 levels deep for quick estimation
		await countFiles(rootStat, 0, 2);
	} catch (error) {
		// If there's an error, assume small workspace
		console.warn(`Error estimating workspace size: ${error}`);
	}

	return { fileCount, exceededLimit };
};

/**
 * Determines appropriate limits based on workspace size
 */
const getWorkspaceLimits = (fileCount: number, exceededLimit: boolean): {
	maxDepth: number;
	maxItemsPerDir: number;
	isLargeWorkspace: boolean;
} => {
	// If we exceeded the limit during counting, it's definitely a very large workspace
	if (exceededLimit || fileCount >= WORKSPACE_SIZE_LARGE) {
		return {
			maxDepth: RESTRICTIVE_MAX_DEPTH_VERY_LARGE,
			maxItemsPerDir: RESTRICTIVE_MAX_ITEMS_PER_DIR_VERY_LARGE,
			isLargeWorkspace: true
		};
	}

	if (fileCount >= WORKSPACE_SIZE_MEDIUM) {
		return {
			maxDepth: RESTRICTIVE_MAX_DEPTH_LARGE,
			maxItemsPerDir: RESTRICTIVE_MAX_ITEMS_PER_DIR_LARGE,
			isLargeWorkspace: true
		};
	}

	if (fileCount >= WORKSPACE_SIZE_SMALL) {
		return {
			maxDepth: RESTRICTIVE_MAX_DEPTH_MEDIUM,
			maxItemsPerDir: RESTRICTIVE_MAX_ITEMS_PER_DIR_MEDIUM,
			isLargeWorkspace: true
		};
	}

	// Small workspace - use normal restrictive limits (not the very restrictive ones)
	return {
		maxDepth: RESTRICTIVE_MAX_DEPTH_SMALL,
		maxItemsPerDir: RESTRICTIVE_MAX_ITEMS_PER_DIR_SMALL,
		isLargeWorkspace: false
	};
};
...
class DirectoryStrService extends Disposable implements IDirectoryStrService {
	...
	// Cache for getDirectoryStrTool: URI -> CacheEntry
	private readonly directoryStrToolCache = new Map<string, CacheEntry>();
	// Cache for getAllDirectoriesStr: workspace key -> CacheEntry
	private readonly allDirectoriesStrCache = new Map<string, CacheEntry>();
	...
	constructor(
		...
		this._register(this.fileService.onDidFilesChange(e => {
			this.handleFileSystemChange(e);
		}));

		// Listen to workspace folder changes to invalidate cache
		this._register(this.workspaceContextService.onDidChangeWorkspaceFolders(() => {
			this.invalidateCache();
		}));
	}

	private handleFileSystemChange(e: FileChangesEvent): void {
		const workspaceFolders = this.workspaceContextService.getWorkspace().folders;
		if (workspaceFolders.length === 0) {
			return;
		}

		// Check if any changed file is within the workspace
		const allChangedUris = [...e.rawAdded, ...e.rawUpdated, ...e.rawDeleted];
		let shouldInvalidate = false;

		for (const changedUri of allChangedUris) {
			// Check if the changed file is within any workspace folder
			for (const folder of workspaceFolders) {
				if (changedUri.toString().startsWith(folder.uri.toString())) {
	...
	invalidateCache(): void {
		this.directoryStrToolCache.clear();
		this.allDirectoriesStrCache.clear();
	}
	...
	async getDirectoryStrTool(uri: URI) {
		...
		const cached = this.directoryStrToolCache.get(cacheKey);
		...
		// Store in cache
		this.directoryStrToolCache.set(cacheKey, {
			value: c,
			timestamp: Date.now()
		});
	...
	async getAllDirectoriesStr({ cutOffMessage, }: { cutOffMessage: string, }) {
		...
		// Check cache first
		const cacheKey = this.getWorkspaceCacheKey();
		const cached = this.allDirectoriesStrCache.get(cacheKey);
		if (cached && this.isCacheEntryValid(cached)) {
			// Note: cutOffMessage might change, but we'll use cached value for performance
			// If cutOffMessage is critical, we could include it in the cache key
			return cached.value;
		}
		...
		// Detect workspace size before generating context
		// Check the first folder (main workspace) to determine size
		let workspaceLimits: { maxDepth: number; maxItemsPerDir: number; isLargeWorkspace: boolean } | null = null;
		if (folders.length > 0) {
			const mainFolder = folders[0];
			const { fileCount, exceededLimit } = await estimateWorkspaceSize(mainFolder.uri, this.fileService);
			workspaceLimits = getWorkspaceLimits(fileCount, exceededLimit);

			// If workspace is large, log a warning and use restrictive limits from the start
			if (workspaceLimits.isLargeWorkspace) {
				console.log(`Large workspace detected (${fileCount}${exceededLimit ? '+' : ''} files). Applying restrictive limits: maxDepth=${workspaceLimits.maxDepth}, maxItemsPerDir=${workspaceLimits.maxItemsPerDir}`);
			}
		}
		...
		for (let i = 0; i < folders.length; i += 1) {
			...
			// Estimate size for this specific folder if not already done or for multi-folder workspaces
			let folderLimits = workspaceLimits;
			if (!folderLimits || i > 0) {
				const { fileCount, exceededLimit } = await estimateWorkspaceSize(rootURI, this.fileService);
				folderLimits = getWorkspaceLimits(fileCount, exceededLimit);
			}

			// For large workspaces, skip the initial START_MAX_DEPTH attempt and go directly to restrictive limits
			let content: string;
			let wasCutOff: boolean;

			if (folderLimits.isLargeWorkspace) {
				// For large workspaces, use restrictive limits directly
				const result = await computeAndStringifyDirectoryTree(
					eRoot,
					this.fileService,
					MAX_DIRSTR_CHARS_TOTAL_BEGINNING - str.length,
					{ count: 0 },
					{ maxDepth: folderLimits.maxDepth, currentDepth: 0, maxItemsPerDir: folderLimits.maxItemsPerDir }
				);
				content = result.content;
				wasCutOff = result.wasCutOff;
			} else {
				// For smaller workspaces, try with START_MAX_DEPTH first
				const { content: initialContent, wasCutOff: initialCutOff } = await computeAndStringifyDirectoryTree(
					eRoot,
					this.fileService,
					MAX_DIRSTR_CHARS_TOTAL_BEGINNING - str.length,
		...
		// Store in cache (note: we cache without cutOffMessage, so it might need adjustment if cutOffMessage varies)
		this.allDirectoriesStrCache.set(cacheKey, {
			value: ans,
			timestamp: Date.now()
		});
