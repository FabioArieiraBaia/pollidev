...
export const defaultProviderSettings = {
	...
	pollinations: {
		apiKey: '', // Aceita tanto pk_... (chave pública) quanto sk_... (chave privada)
	},
...
export const defaultModelsOfProvider = {
	...
	pollinations: [ // https://enter.pollinations.ai/api/docs - Ordered by context window (largest to smallest)
		'nova-micro', // 32K
		'qwen-coder', // 22K
		'mistral', // 13K
		'gemini-fast', // 12K
		...
		'deepseek', // 840
		'glm', // 640
		'kimi-k2-thinking', // 570
		'midijourney', // 550
		'claude', // 330
		'claude-large', // 200
		'perplexity-reasoning', // 180
		'gemini-large', // 120
		'openai-large', // 100
		'openai-audio', // 4.2 min audio
	],
...
export type VoidStaticModelInfo = { // not stateful
	...
	// vision capabilities
	supportsVision?: boolean; // whether the model supports vision/image understanding (for browser navigation)

	// audio capabilities
	supportsAudio?: boolean; // whether the model supports audio input/output

	// video capabilities
	supportsVideo?: boolean; // whether the model supports video input

	// tool support
	supportsTools?: boolean; // whether the model supports function calling/tools (some models like perplexity don't)
...
const extensiveModelOptionsFallback: VoidStaticProviderInfo['modelOptionsFallback'] = (modelName, fallbackKnownValues) => {
	...
	if (lower.includes('gemini') && (lower.includes('2.5') || lower.includes('2-5'))) {
		...
		if (lower.includes('flash') && lower.includes('lite')) return toFallback(geminiModelOptions, 'gemini-2.5-flash-lite')
	...
	// Gemini 2.0.x models
	if (lower.includes('gemini') && (lower.includes('2.0') || lower.includes('2-0') || (lower.includes('2') && !lower.includes('2.5') && !lower.includes('2-5')))) {
		if (lower.includes('thinking')) return toFallback(geminiModelOptions, 'gemini-2.0-flash-thinking-exp')
		if (lower.includes('flash')) return toFallback(geminiModelOptions, 'gemini-2.0-flash-exp')
		return toFallback(geminiModelOptions, 'gemini-2.0-flash-exp')
	}
...
const geminiSettings: VoidStaticProviderInfo = {
	...
	modelOptionsFallback: (modelName) => {
		...
		if (lower.includes('gemini') && (lower.includes('2.5') || lower.includes('2-5'))) {
			if (lower.includes('pro')) return { modelName: 'gemini-2.5-pro', recognizedModelName: 'gemini-2.5-pro', ...geminiModelOptions['gemini-2.5-pro'] }
			if (lower.includes('flash') && lower.includes('lite')) return { modelName: 'gemini-2.5-flash-lite', recognizedModelName: 'gemini-2.5-flash-lite', ...geminiModelOptions['gemini-2.5-flash-lite'] }
			if (lower.includes('flash')) return { modelName: 'gemini-2.5-flash', recognizedModelName: 'gemini-2.5-flash', ...geminiModelOptions['gemini-2.5-flash'] }
			// Default to flash for gemini-2.5
			return { modelName: 'gemini-2.5-flash', recognizedModelName: 'gemini-2.5-flash', ...geminiModelOptions['gemini-2.5-flash'] }
		}
		// Recognize Gemini 2.0.x models
		if (lower.includes('gemini') && (lower.includes('2.0') || lower.includes('2-0') || (lower.includes('2') && !lower.includes('2.5') && !lower.includes('2-5')))) {
			if (lower.includes('thinking')) return { modelName: 'gemini-2.0-flash-thinking-exp', recognizedModelName: 'gemini-2.0-flash-thinking-exp', ...geminiModelOptions['gemini-2.0-flash-thinking-exp'] }
			if (lower.includes('flash')) return { modelName: 'gemini-2.0-flash-exp', recognizedModelName: 'gemini-2.0-flash-exp', ...geminiModelOptions['gemini-2.0-flash-exp'] }
			// Default to flash-exp for gemini-2.0
			return { modelName: 'gemini-2.0-flash-exp', recognizedModelName: 'gemini-2.0-flash-exp', ...geminiModelOptions['gemini-2.0-flash-exp'] }
		}
		// Generic Gemini fallback
		if (lower.includes('gemini')) {
			if (lower.includes('pro')) return { modelName: 'gemini-2.5-pro', recognizedModelName: 'gemini-2.5-pro', ...geminiModelOptions['gemini-2.5-pro'] }
		...
		// Se contém "gemini" mas não foi reconhecido acima, garantir gemini-style para ferramentas funcionarem
		if (lower.includes('gemini')) {
			return {
				modelName,
...
// ---------------- POLLINATIONS.AI ----------------
// Updated with correct capabilities from https://enter.pollinations.ai/api/docs
// Capabilities: supportsVision (image), supportsAudio (audio), supportsVideo (video), supportsTools (function calling), reasoning
const pollinationsModelOptions = {
	// === Text-only models ===
	'nova-micro': {
		contextWindow: 32_000,
		reservedOutputTokenSpace: 4_096,
	...
	'gemini-search': {
		...
		reasoningCapabilities: false,
		supportsVision: true,
		supportsTools: false, // NO tools support per API docs
		cost: { input: 0, output: 0 },
		downloadable: false,
	},
	'claude-fast': {
	...
	// === Audio + Vision models ===
	'openai-audio': {
		contextWindow: 128_000, // 4.2 min audio context
		reservedOutputTokenSpace: 16_384,
		supportsFIM: false,
		supportsSystemMessage: 'system-role',
		specialToolFormat: 'openai-style',
	...
	'glm': {
		contextWindow: 198_000, // CORRECTED: 198K from API docs
		reservedOutputTokenSpace: 8_192,
		supportsFIM: false,
		supportsSystemMessage: 'system-role',
		specialToolFormat: 'openai-style',
	...
	'minimax': {
		contextWindow: 200_000, // CORRECTED: 200K from API docs
		reservedOutputTokenSpace: 8_192,
		supportsFIM: false,
		supportsSystemMessage: 'system-role',
		specialToolFormat: 'openai-style',
	...
	// === Search models (no tools) ===
	'perplexity-fast': {
		contextWindow: 4_100,
		reservedOutputTokenSpace: 820,
		supportsFIM: false,
		supportsSystemMessage: 'system-role',
		specialToolFormat: 'openai-style',
		reasoningCapabilities: false,
...
const pollinationsSettings: VoidStaticProviderInfo = {
	modelOptions: pollinationsModelOptions,
	modelOptionsFallback: (modelName) => {
		const res = extensiveModelOptionsFallback(modelName)
		// Pollinations is OpenAI-compatible, use openai-style
		if (res?.specialToolFormat === 'gemini-style') {
	...
	providerReasoningIOSettings: {
		// OpenAI-compatible reasoning support
		input: { includeInPayload: openAICompatIncludeInPayloadReasoning },
		output: { nameOfFieldInDelta: 'reasoning' },
	},
}
...
/**
 * Returns a list of model names that support vision capabilities for the given provider.
 * @param provider Optional provider name. If not provided, returns models from all providers.
 * @returns Array of model names that support vision
 */
export const getModelsWithVision = (provider?: ProviderName): string[] => {
	const models: string[] = []
	
	if (provider) {
		// Get models for specific provider
		const providerInfo = modelSettingsOfProvider[provider]
		if (providerInfo) {
			for (const [modelName, modelInfo] of Object.entries(providerInfo.modelOptions)) {
				if (modelInfo.supportsVision === true) {
		...
		for (const providerInfo of Object.values(modelSettingsOfProvider)) {
			for (const [modelName, modelInfo] of Object.entries(providerInfo.modelOptions)) {
				if (modelInfo.supportsVision === true) {
					models.push(modelName)
				}
...
/**
 * Returns a list of model names that support audio capabilities for the given provider.
 * @param provider Optional provider name. If not provided, returns models from all providers.
 * @returns Array of model names that support audio
 */
export const getModelsWithAudio = (provider?: ProviderName): string[] => {
	const models: string[] = []
	...
	if (provider) {
		...
		if (providerInfo) {
			for (const [modelName, modelInfo] of Object.entries(providerInfo.modelOptions)) {
				if (modelInfo.supportsAudio === true) {
					models.push(modelName)
				}
			}
		}
...
/**
 * Returns a list of model names that support video capabilities for the given provider.
 * @param provider Optional provider name. If not provided, returns models from all providers.
 * @returns Array of model names that support video
 */
export const getModelsWithVideo = (provider?: ProviderName): string[] => {
	const models: string[] = []
	...
	if (provider) {
		...
		if (providerInfo) {
			for (const [modelName, modelInfo] of Object.entries(providerInfo.modelOptions)) {
				if (modelInfo.supportsVideo === true) {
					models.push(modelName)
				}
			}
		}
...
export const modelSupportsCapability = (
	providerName: ProviderName,
	modelName: string,
	capability: 'vision' | 'audio' | 'video' | 'tools'
): boolean => {
	const caps = getModelCapabilities(providerName, modelName, undefined)
	switch (capability) {
		case 'vision': return caps.supportsVision === true
		case 'audio': return caps.supportsAudio === true
		case 'video': return caps.supportsVideo === true
		case 'tools': return caps.supportsTools !== false // default to true if undefined
	}
}
