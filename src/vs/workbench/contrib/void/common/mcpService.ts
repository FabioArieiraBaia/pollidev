...
class MCPService extends Disposable implements IMCPService {
	...
	public getMCPTools(): InternalToolInfo[] | undefined {
		try {
			...
			// #region agent log
			fetch('http://127.0.0.1:7243/ingest/1ce6e17d-b708-4230-aa86-6bd5be848bbc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'mcpService.ts:186',message:'getMCPTools: start',data:{hasState:!!this.state,hasServers:!!this.state?.mcpServerOfName,serverCount:Object.keys(this.state?.mcpServerOfName||{}).length,serverNames:Object.keys(this.state?.mcpServerOfName||{}),serverStatuses:Object.entries(this.state?.mcpServerOfName||{}).map(([n,s])=>({name:n,status:s?.status,toolsCount:s?.tools?.length||0}))},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
			// #endregion
			
			// Validate state exists
			if (!this.state || !this.state.mcpServerOfName) {
				// #region agent log
				fetch('http://127.0.0.1:7243/ingest/1ce6e17d-b708-4230-aa86-6bd5be848bbc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'mcpService.ts:193',message:'getMCPTools: no state or servers',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
				// #endregion
				return undefined
			}
			...
			for (const serverName in this.state.mcpServerOfName) {
				...
				// #region agent log
				fetch('http://127.0.0.1:7243/ingest/1ce6e17d-b708-4230-aa86-6bd5be848bbc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'mcpService.ts:199',message:'getMCPTools: checking server',data:{serverName,hasServer:!!server,status:server?.status,toolsCount:server?.tools?.length||0,isBrowserServer:serverName.includes('browser')},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
				// #endregion
				
				// Skip servers that are not in a valid state
				if (!server || server.status === 'error' || server.status === 'loading') {
					continue
				}
				...
				server.tools.forEach(tool => {
					...
					if (!tool || typeof tool !== 'object') {
						console.warn(`Invalid tool from server ${serverName}: tool is not an object`, tool)
						return
					}
					
					if (!tool.name || typeof tool.name !== 'string' || tool.name.length === 0) {
						console.warn(`Invalid tool from server ${serverName}: missing or invalid name`, tool)
						return
					}
			...
			// #region agent log
			fetch('http://127.0.0.1:7243/ingest/1ce6e17d-b708-4230-aa86-6bd5be848bbc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'mcpService.ts:229',message:'getMCPTools: result',data:{allToolsCount:allTools.length,toolNames:allTools.map(t=>t.name),hasBrowserTools:allTools.some(t=>t.name?.includes('browser'))},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
			// #endregion
