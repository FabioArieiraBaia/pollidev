	...
	if (providerName === 'openAI') {
		...
		return new OpenAI({
			...
			defaultHeaders: {
				...
				'X-Title': 'PolliDev', // Optional. Shows in rankings on openrouter.ai.
			},
			...commonPayloadOpts,
		})
	}
...
/**
 * Validate MCP tools structure and filter out invalid entries
 * This ensures that only properly formatted tools are passed to the LLM
 */
const _validateMCPTools = (mcpTools: InternalToolInfo[] | undefined): InternalToolInfo[] | undefined => {
	if (!mcpTools) {
		return undefined
	}
	
	if (!Array.isArray(mcpTools)) {
		console.warn('MCP tools is not an array:', typeof mcpTools)
		return undefined
	}
	
	// Filter and validate each tool
...
const openAITools = (chatMode: ChatMode | null, mcpTools: InternalToolInfo[] | undefined, includeThoughtSignature: boolean = false) => {
	const allowedTools = availableTools(chatMode, mcpTools)
	if (!allowedTools || allowedTools.length === 0) {
		// #region agent log
		fetch('http://127.0.0.1:7243/ingest/1ce6e17d-b708-4230-aa86-6bd5be848bbc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'sendLLMMessage.impl.ts:277',message:'openAITools: no allowed tools',data:{chatMode,mcpToolsCount:mcpTools?.length||0},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
		// #endregion
		return null
	}
	...
	for (const toolInfo of allowedTools) {
		const tool = toOpenAICompatibleTool(toolInfo)
		// Add thought_signature to each tool when needed (for Gemini via Pollinations)
		if (includeThoughtSignature) {
			(tool as any).thought_signature = true
		}
		openAITools.push(tool)
	}
	
	// #region agent log
	fetch('http://127.0.0.1:7243/ingest/1ce6e17d-b708-4230-aa86-6bd5be848bbc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'sendLLMMessage.impl.ts:289',message:'openAITools: result',data:{chatMode,openAIToolsCount:openAITools.length,toolNames:openAITools.map(t=>t.function.name),hasBrowserTools:openAITools.some(t=>t.function.name?.includes('browser'))},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
	// #endregion
...
/**
 * Valida que as ferramentas MCP foram corretamente incluídas no payload do LLM.
 * Verifica especialmente ferramentas do navegador que devem estar disponíveis.
 * @param mcpTools - Ferramentas MCP fornecidas
 * @param chatMode - Modo de chat atual
 * @param payloadTools - Ferramentas incluídas no payload (pode ser array ou objeto com array)
 * @param providerName - Nome do provedor para logging
 */
const validateMCPToolsInPayload = (
	mcpTools: InternalToolInfo[] | undefined,
	chatMode: ChatMode | null,
	payloadTools: any,
	providerName: ProviderName
): void => {
	// Só valida se estivermos em modo agent (onde MCP tools devem estar disponíveis)
	if (chatMode !== 'agent' || !mcpTools || mcpTools.length === 0) {
		return
	}

	// Extrai nomes das ferramentas do payload
	let payloadToolNames: string[] = []
	
	if (Array.isArray(payloadTools)) {
		// Pode ser array de ferramentas (OpenAI, Anthropic) ou array de objetos Gemini com functionDeclarations
		for (const tool of payloadTools) {
			// OpenAI-style: tool.function.name
			if (tool?.function?.name) {
				payloadToolNames.push(tool.function.name)
			}
			// Anthropic-style: tool.name
			else if (tool?.name) {
				payloadToolNames.push(tool.name)
			}
			// Gemini-style: array de objetos com functionDeclarations
			else if (tool?.functionDeclarations && Array.isArray(tool.functionDeclarations)) {
				for (const fd of tool.functionDeclarations) {
					if (fd?.name) {
						payloadToolNames.push(fd.name)
					}
				}
			}
		}
	} else if (payloadTools?.tools && Array.isArray(payloadTools.tools)) {
		// Payload pode ter estrutura { tools: [...] }
		for (const tool of payloadTools.tools) {
			if (tool?.function?.name) {
				payloadToolNames.push(tool.function.name)
			} else if (tool?.name) {
				payloadToolNames.push(tool.name)
	...
	} else if (payloadTools?.functionDeclarations && Array.isArray(payloadTools.functionDeclarations)) {
		// Gemini pode ter functionDeclarations diretamente
		for (const fd of payloadTools.functionDeclarations) {
			if (fd?.name) {
				payloadToolNames.push(fd.name)
			}
		}
	...
	// Verifica se ferramentas MCP esperadas estão no payload
	const mcpToolNames = mcpTools.map(t => t.name)
	const missingTools = mcpToolNames.filter(name => !payloadToolNames.includes(name))

	// Verifica especialmente ferramentas do navegador
	const browserTools = mcpTools.filter(t => t.name.startsWith('mcp_cursor-ide-browser_'))
	const missingBrowserTools = browserTools.filter(t => !payloadToolNames.includes(t.name))

	if (missingTools.length > 0) {
		console.warn(`[Void] Algumas ferramentas MCP não foram incluídas no payload para ${providerName}:`, missingTools)
	}

	if (missingBrowserTools.length > 0) {
		console.warn(`[Void] Ferramentas do navegador não foram incluídas no payload para ${providerName}:`, missingBrowserTools.map(t => t.name))
	}

	// Se há ferramentas MCP mas nenhuma está no payload, isso é um problema
	if (mcpToolNames.length > 0 && payloadToolNames.length === 0 && chatMode === 'agent') {
		console.warn(`[Void] Aviso: Modo agent ativo com ${mcpToolNames.length} ferramenta(s) MCP, mas nenhuma ferramenta foi incluída no payload para ${providerName}.`)
	}
}
...
const _sendOpenAICompatibleChat = async ({ messages, onText, onFinalMessage, onError, settingsOfProvider, modelSelectionOptions, modelName: modelName_, _setAborter, providerName, chatMode, separateSystemMessage, overridesOfModel, mcpTools }: SendChatParams_Internal) => {
	// #region agent log
	fetch('http://127.0.0.1:7243/ingest/1ce6e17d-b708-4230-aa86-6bd5be848bbc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'sendLLMMessage.impl.ts:412',message:'_sendOpenAICompatibleChat: entry',data:{providerName,modelName:modelName_,chatMode},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
	// #endregion
	const {
		modelName,
		specialToolFormat,
	...
	// #region agent log
	fetch('http://127.0.0.1:7243/ingest/1ce6e17d-b708-4230-aa86-6bd5be848bbc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'sendLLMMessage.impl.ts:418',message:'_sendOpenAICompatibleChat: after getModelCapabilities',data:{modelName,specialToolFormat,providerName},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
	// #endregion
	...
	// Detect if we need thought_signature for Gemini models via Pollinations
	// When reasoning is enabled + tools are present, Vertex AI requires thought_signature
	const isPollinationsGeminiWithReasoning = providerName === 'pollinations' && 
		modelName.startsWith('gemini') && 
		reasoningInfo?.isReasoningEnabled;

	// tools - pass includeThoughtSignature flag
	// Validate mcpTools before passing to openAITools
	const validatedMcpTools = _validateMCPTools(mcpTools)
	
	// #region agent log
	fetch('http://127.0.0.1:7243/ingest/1ce6e17d-b708-4230-aa86-6bd5be848bbc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'sendLLMMessage.impl.ts:429',message:'_sendOpenAICompatibleChat: before openAITools',data:{chatMode,mcpToolsCount:mcpTools?.length||0,validatedMcpToolsCount:validatedMcpTools?.length||0,specialToolFormat,providerName},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
	// #endregion
	
	const potentialTools = openAITools(chatMode, validatedMcpTools, isPollinationsGeminiWithReasoning)
	const nativeToolsObj = potentialTools && specialToolFormat === 'openai-style' ?
		{ tools: potentialTools } as const
		: {}
	
	// #region agent log
	fetch('http://127.0.0.1:7243/ingest/1ce6e17d-b708-4230-aa86-6bd5be848bbc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'sendLLMMessage.impl.ts:435',message:'_sendOpenAICompatibleChat: after openAITools',data:{hasPotentialTools:!!potentialTools,potentialToolsCount:potentialTools?.length||0,hasNativeToolsObj:!!nativeToolsObj.tools,toolsInPayload:!!nativeToolsObj.tools},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
	// #endregion

	// Also add thought_signature in payload root (required by Vertex AI)
	const thoughtSignaturePayload = isPollinationsGeminiWithReasoning && potentialTools && potentialTools.length > 0 ? {
		thought_signature: true
	} : {};

	// Validação: garantir que ferramentas MCP foram incluídas no payload
	if (specialToolFormat === 'openai-style') {
		validateMCPToolsInPayload(validatedMcpTools, chatMode, nativeToolsObj.tools, providerName)
	}
	...
	// #region agent log
	fetch('http://127.0.0.1:7243/ingest/1ce6e17d-b708-4230-aa86-6bd5be848bbc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'sendLLMMessage.impl.ts:478',message:'_sendOpenAICompatibleChat: options payload',data:{modelName,specialToolFormat,hasTools:!!nativeToolsObj.tools,toolsCount:nativeToolsObj.tools?.length||0,hasThoughtSignature:!!thoughtSignaturePayload.thought_signature,toolsNames:nativeToolsObj.tools?.map((t:any)=>t.function?.name)||[]},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'D'})}).catch(()=>{});
	// #endregion
	...
	if (!specialToolFormat) {
		const { newOnText, newOnFinalMessage } = extractXMLToolsWrapper(onText, onFinalMessage, chatMode, validatedMcpTools)
		onText = newOnText
		onFinalMessage = newOnFinalMessage
	}
...
const sendAnthropicChat = async ({ messages, providerName, onText, onFinalMessage, onError, settingsOfProvider, modelSelectionOptions, overridesOfModel, modelName: modelName_, _setAborter, separateSystemMessage, chatMode, mcpTools }: SendChatParams_Internal) => {
	...
	// tools
	// Validate mcpTools before passing to anthropicTools
	const validatedMcpTools = _validateMCPTools(mcpTools)
	const potentialTools = anthropicTools(chatMode, validatedMcpTools)
...
const sendGeminiChat = async ({
	...
	// tools
	// Validate mcpTools before passing to geminiTools
	const validatedMcpTools = _validateMCPTools(mcpTools)
	const potentialTools = geminiTools(chatMode, validatedMcpTools)
